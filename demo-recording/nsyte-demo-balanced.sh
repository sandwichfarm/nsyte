#!/bin/bash
# Balanced timing version of nsyte CLI demo (~45 seconds total)
# Natural pacing that allows viewers to read and understand
...
# Helper functions with natural timing
type_command() {
    printf "$ "
    for (( i=0; i<${#1}; i++ )); do
        printf "${1:$i:1}"
        sleep 0.08  # Slower typing for commands
    done
    printf "\n"
    sleep 0.8  # Pause after command
}

type_input() {
    for (( i=0; i<${#1}; i++ )); do
        printf "${1:$i:1}"
        sleep 0.05  # Natural typing speed
    done
    printf "\n"
    sleep 0.4  # Natural pause after input
}

show_output() {
    printf "%s\n" "$1"
    sleep 0.15  # Small pause between lines
}
...
sleep 1.2  # Time to read options
show_output ""
...
# QR Code - visible for 5 seconds (enough time to understand)
cat << 'EOF'
    ██████████████    ██████  ██        ██      ██████████████    
    ██          ██        ████  ██  ██      ██  ██          ██    
    ██  ██████  ██    ██████    ████      ████  ██  ██████  ██    
...
# Simulated wait with countdown
sleep 2
printf "\rWaiting for connection... "
sleep 1
printf "\rConnecting to bunker...   "
sleep 1
printf "\rEstablishing secure connection..."
sleep 1.5
printf "\r                                        \r"
...
show_output ""
sleep 1  # Pause to absorb connection success
...
show_output ""
sleep 1.5  # Pause before next command
...
sleep 3  # Final pause to read help menu