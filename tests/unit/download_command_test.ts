...
Deno.test("Download Command - Options Validation", async (t) => {
  ...
  await t.step("should validate relay URLs", () => {
    ...
    assertEquals(validateRelays("wss://relay1.com,wss://relay2.com"), [
      "wss://relay1.com",
      "wss://relay2.com",
    ]);
    assertEquals(validateRelays("wss://relay.com, ws://localhost:8080"), [
      "wss://relay.com",
      "ws://localhost:8080",
    ]);
  ...
  await t.step("should validate public key formats", () => {
    ...
    assertEquals(
      validatePubkey("npub1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq92l2sj"),
      { type: "npub", value: "npub1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq92l2sj" },
    );
    assertEquals(
      validatePubkey("1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef"),
  ...
  await t.step("should validate private key formats", () => {
    ...
    assertEquals(
      validatePrivateKey("nsec1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqsscfuwn"),
...
Deno.test("Download Command - Workflow Logic", async (t) => {
  await t.step("should validate authentication requirements", () => {
    const validateAuth = (
      options: { privatekey?: string; bunker?: string; nbunksec?: string; pubkey?: string },
    ) => {
      ...
      if (!hasPrivateAuth && !hasPubkeyOnly) {
        throw new Error(
          "Must provide either authentication (private key/bunker) or a public key to download from",
        );
      }

      if (hasPrivateAuth && options.pubkey) {
        throw new Error(
          "Cannot specify both authentication and pubkey - use auth to download your own files, or pubkey to download someone else's",
        );
      }
    ...
    // Valid cases
    assertEquals(validateAuth({ privatekey: "nsec123..." }), {
      mode: "authenticated",
      canDownloadPrivate: true,
    });
    assertEquals(validateAuth({ bunker: "bunker://..." }), {
      mode: "authenticated",
      canDownloadPrivate: true,
    });
    assertEquals(validateAuth({ nbunksec: "nbunk..." }), {
      mode: "authenticated",
      canDownloadPrivate: true,
    });
    assertEquals(validateAuth({ pubkey: "npub123..." }), {
      mode: "public",
...
Deno.test("Download Command - Error Scenarios", async (t) => {
  ...
  await t.step("should handle malformed file data", () => {
    const validateFileData = (fileData: { content?: Uint8Array; hash?: string; size?: number }) => {
      ...
      if (
        fileData.size !== undefined && fileData.content && fileData.content.length !== fileData.size
      ) {
