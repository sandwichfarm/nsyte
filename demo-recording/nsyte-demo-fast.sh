#!/bin/bash
# Fast version of nsyte CLI demo for recording
...
# Helper functions with faster timing
type_command() {
    printf "$ "
    for (( i=0; i<${#1}; i++ )); do
        printf "${1:$i:1}"
...
# QR Code for https://www.youtube.com/watch?v=zL19uMsnpSU - visible for 5 seconds for fast demo
cat << 'EOF'
    ██████████████    ██████  ██        ██      ██████████████    
    ██          ██        ████  ██  ██      ██  ██          ██    
    ██  ██████  ██    ██████    ████      ████  ██  ██████  ██    
