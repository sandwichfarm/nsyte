...
export function handleDeleteConfirmation(
  ...
  if (sequence) {
    ...
    if (state.deleteConfirmText.toLowerCase() === "yes") {
      ...
      // Update status
      state.status = `Deleting ${filesToDelete.length} file${
      ...
      deleteFiles(filesToDelete, state).then((success) => {
        if (success) {
          ...
          // Update status
          state.status = `Deleted ${filesToDelete.length} file${
            filesToDelete.length > 1 ? "s" : ""
          }. Verifying...`;
...
export async function deleteFiles(
  ...
  try {
    ...
    if (hasBlossomServers) {
      ...
      state.status = `Deleting from ${allServers.size} Blossom server${
        allServers.size > 1 ? "s" : ""
      }...`;
      state.statusColor = colors.cyan;
      render(state);
      ...
      for (const [hash, servers] of fileHashMap.entries()) {
        for (const server of servers) {
          try {
            state.status = `Deleting from ${
              server.replace(/^https?:\/\//, "").substring(0, 20)
            }...`;
            render(state);
      ...
      if (blossomDeletedCount > 0) {
        state.status = `Deleted from ${blossomDeletedCount} Blossom server${
          blossomDeletedCount > 1 ? "s" : ""
        }`;
        state.statusColor = colors.green;
      } else {
    ...
    try {
      ...
      const publishPromises = relayArray.map(async (relay) => {
        try {
          ...
          const results = await pool.publish([relay], deleteEvent, { retries: 1, timeout: 5000 });
      ...
      if (acceptedCount === 0) {
        ...
        if (blossomDeletedCount > 0) {
          state.status =
            `Blossom deletion succeeded but relay deletion failed: All ${rejectedCount} relay${
              rejectedCount > 1 ? "s" : ""
            } rejected deletion of ${fileCount} file${fileCount > 1 ? "s" : ""}`;
          state.statusColor = colors.yellow;
        } else {
          state.status = `Delete failed: All ${rejectedCount} relay${
            rejectedCount > 1 ? "s" : ""
          } rejected deletion of ${fileCount} file${fileCount > 1 ? "s" : ""}`;
          state.statusColor = colors.red;
        }
        ...
        log.warn(
          `Delete event accepted by ${acceptedCount} relay(s), rejected by ${rejectedCount}`,
        );
        const fileCount = eventIds.length;
        if (blossomDeletedCount > 0) {
          state.status =
            `Partial success: ${blossomDeletedCount} Blossom deleted, ${acceptedCount} relay${
              acceptedCount > 1 ? "s" : ""
            } accepted, ${rejectedCount} rejected`;
          state.statusColor = colors.yellow;
        } else {
          state.status = `Partial success for ${fileCount} file${
            fileCount > 1 ? "s" : ""
          }: ${acceptedCount} relay${
            acceptedCount > 1 ? "s" : ""
          } accepted, ${rejectedCount} rejected`;
        ...
        // All relays accepted
        if (blossomDeletedCount > 0) {
          state.status = `Successfully deleted from ${blossomDeletedCount} Blossom server${
            blossomDeletedCount > 1 ? "s" : ""
          } and ${acceptedCount} relay${acceptedCount > 1 ? "s" : ""}`;
          state.statusColor = colors.green;
        } else {
          state.status = `Successfully deleted from ${acceptedCount} relay${
            acceptedCount > 1 ? "s" : ""
          }`;
          state.statusColor = colors.green;
        }
      ...
      if (blossomDeletedCount > 0) {
        state.status = `Blossom deletion succeeded but relay deletion failed: ${
          error instanceof Error ? error.message : "Relay communication error"
        }`;
        state.statusColor = colors.yellow;
      } else {
...
async function verifyDeletion(files: FileEntryWithSources[], state: BrowseState): Promise<void> {
  try {
    ...
    if (fullyDeleted.length > 0) {
      ...
      state.status = `Warning: ${partiallyDeleted.length} file${
        partiallyDeleted.length > 1 ? "s" : ""
      } still on some relays`;
      state.statusColor = colors.yellow;
      render(state);
      ...
      state.status = `Delete accepted but ${notDeleted.length} file${
        notDeleted.length > 1 ? "s" : ""
      } still exist on all relays`;
      state.statusColor = colors.red;
      render(state);
...
export function handleListModeKey(state: BrowseState, key: string): boolean {
  ...
  if (key && key.length > 0) {
    log.debug(
      `Processing key event: "${key}" (length: ${key.length}, codes: ${
