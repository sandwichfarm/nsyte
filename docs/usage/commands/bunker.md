...
The top-level `nsyte bunker` command takes no per-command options of its own (only globals). The
options below apply to the `connect` subcommand (`nsyte bunker connect`):
...
- `--no-persist` — Don't store nbunksec (ephemeral connection for CI/CD; auto-generated by Cliffy as the negation of `--persist`)
- `--force-encrypted-storage` — Force encrypted file storage instead of OS keychain (default: `false`)
