    ...
    while (true) {
      ...
      while (true) {
        ...
        if (sites.length > 1) {
          ...
          selectedSiteName = site.title || (site.type === "root" ? "Root Site" : site.identifier!);
        }
        ...
        const handleResize = () => {
          ...
          resizeTimeout = setTimeout(() => {
            ...
            const maxIndex = Math.min((state.page + 1) * state.pageSize, state.treeItems.length) -
              1;
      ...
      // Continue the outer loop - the identity menu will be shown at the top
      continue;
