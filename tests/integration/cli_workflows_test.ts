...
Deno.test("CLI Workflows - Command Validation", async (t) => {
  ...
  await t.step("should validate global options", () => {
    ...
    assertThrows(() => validateOptions({ nonInteractive: true, interactive: true }), Error, "non-interactive and interactive");
  });
});
  ...
  await t.step("should track command execution progress", () => {
    ...
    const createProgressTracker = () => {
      ...
      return {
        ...
        setCurrent: (current: string) => { progress.current = current; },
