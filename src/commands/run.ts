      ...
      try {
        ...
        if (!file || !file.sha256) {
          ...
          if (notFoundFile && notFoundFile.sha256) {
            ...
            const elapsed = Math.round(performance.now() - startTime);
            console.log(
              colors.red(`  → File not found: ${requestedPath} (no 404.html) - ${elapsed}ms`),
            );
        ...
        if (!fileData) {
          ...
          if (
            memCached &&
            (!memCached.timestamp || Date.now() - memCached.timestamp < FILE_CACHE_TTL) && !isStale
          ) {
        ...
        if (!fileData || isStale) {
          console.log(
            colors.gray(
              `  → Downloading ${colors.cyan(file.path)}...${isStale ? " (updated)" : ""}`,
            ),
          );
