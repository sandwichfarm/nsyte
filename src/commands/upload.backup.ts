...
export async function uploadCommand(
  ...
  try {
    ...
    let config: import("../lib/config.ts").ProjectConfig;
    ...
    if (options.nonInteractive) {
      ...
      const baseData: Partial<import("../lib/config.ts").ProjectConfig> = existingProjectData || {};
    ...
    if (!config) { // This check might be redundant now but kept for safety.
    ...
    if (options.privatekey) {
      ...
      if (nbunkString) {
        try {
          ...
          if (options.nonInteractive) {
            log.error(\`Failed to use stored nbunksec for ${config.bunkerPubkey} in non-interactive mode. Please run interactively to re-configure or provide key via CLI.\`);
            console.error(colors.red(\`Error with stored bunker for ${config.bunkerPubkey}. See logs.\`));
            Deno.exit(1);
          }
        ...
        // nbunkString is not found for config.bunkerPubkey
        if (options.nonInteractive) {
          log.error(\`No stored secret found for bunker ${config.bunkerPubkey} in non-interactive mode. Please run interactively to configure or provide key via CLI.\`);
          console.error(colors.red(\`Missing secret for configured bunker ${config.bunkerPubkey.slice(0,10)}... See --help.\`));
          Deno.exit(1);
        }
    ...
    if (displayManager.isInteractive()) {
      ...
      console.log(formatConfigValue("Relays", formatRelayList(relays), !options.relays && !config.relays));
      console.log(formatConfigValue("Servers", formatRelayList(servers), !options.servers && !config.servers));
      ...
      console.log(colors.cyan(\`Relays: ${relays.join(", ") || "none"}${!options.relays && !config.relays ? " (default)" : ""}\`));
      console.log(colors.cyan(\`Servers: ${servers.join(", ") || "none"}${!options.servers && !config.servers ? " (default)" : ""}\`));
    ...
    const fallbackFor404 = options.fallback || config.fallback;
    if (fallbackFor404) {
    ...
    if (options.publishRelayList || options.publishServerList || options.publishProfile) {
      ...
      if (options.publishProfile && config.profile) {
        try {
          ...
          const { name, about, picture, website, nip05, lud16, banner } = config.profile;
