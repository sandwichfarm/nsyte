...
export function renderFileList(state: BrowseState) {
    ...
    if (state.filterText) {
      Deno.stdout.writeSync(
        new TextEncoder().encode(
          colors.gray(`[${pathIndicator}] `) + colors.cyan(`(filtered: ${state.filterText})`) +
            "\n",
        ),
      );
    } else {
  ...
  pageItems.forEach((item, listIndex) => {
    ...
    if (item.isDirectory) {
      ...
      const emptyIndicators = " ".repeat(maxRelayCount) + ` ${colors.gray("│")} ` +
        " ".repeat(maxServerCount);
      ...
      console.log(
        `${emptyIndicators} ${extraColumns}${colors.gray(treePrefix)}${colors.gray(dirName + "/")}`,
      );
    } else if (item.file) {
      // Render file
      const fileName = item.path.split("/").pop() || item.path;
      const relativePath = item.file.path.startsWith("/")
        ? item.file.path.substring(1)
        : item.file.path;
      ...
      if (showSizeColumn) {
        if (item.file.size !== undefined) {
          sizeStr = cachedFormatFileSize(item.file.size).padEnd(sizeColumnWidth);
        } else {
      ...
      if (rowBackground && !isDeleting && !isDeleted) {
        ...
        const lineContent =
          `${indicators} ${sizeStr}${typeStr}${treePrefix}${fileName}${hashDisplay}`;
        ...
        const sizeDisplay = showSizeColumn
          ? (isDeleting || isDeleted ? indicatorColor(sizeStr) : colors.gray(sizeStr))
          : "";
        const typeDisplay = showTypeColumn
          ? (isDeleting || isDeleted ? indicatorColor(typeStr) : colors.gray(typeStr))
          : "";
