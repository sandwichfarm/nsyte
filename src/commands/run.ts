      ...
      try {
        ...
        if (!fileListEntry) {
          ...
          (async () => {
            try {
              console.log(
                colors.gray(`  → Fetching file list from ${fileEventRelays.length} relays...`),
              );
              let files = await listRemoteFiles(fileEventRelays, pubkeyHex);

              // Retry with default file relays if allowed and nothing came back (likely relay hiccup)
              if (files.length === 0 && allowFallbackRelays) {
                const fallbackRelays = Array.from(
                  new Set([...fileEventRelays, ...defaultFileRelays]),
                );
                console.log(
                  colors.gray(
          ...
          if (
            ...
            if (fileOption.type === "br") {
              ...
              try {
                ...
                console.log(
                  colors.gray(
                    `  → Decompressed Brotli data: ${formatFileSize(decompressed.byteLength)}`,
                  ),
                );
                ...
                fileCache.set(decompressedCacheKey, {
                  data: decompressed,
                  timestamp: Date.now(),
                  sha256: fileSha256,
                });
              ...
              try {
                ...
                console.log(
                  colors.gray(`  → Decompressed Gzip data: ${formatFileSize(fileData.byteLength)}`),
                );
                successfulFile = tryFile;
 ...
 */
async function saveFileListManifest(
  cacheDir: string | null,
  npub: string,
  files: FileEntry[],
  eventTimestamps?: Map<string, number>,
 ...
 */
async function loadFileListFromDiskCache(
  cacheDir: string | null,
  npub: string,
