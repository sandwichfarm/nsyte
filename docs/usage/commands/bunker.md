...
- `--no-persist` — Don't store nbunksec (ephemeral connection for CI/CD; auto-generated by Cliffy as
  the negation of `--persist`)
- `--force-encrypted-storage` — Force encrypted file storage instead of OS keychain (default:
  `false`)
