...
Deno.test("PrivateKeySigner - getPublicKey", async (t) => {
  await t.step("should return consistent public key", async () => {
  ...
  await t.step("should return valid hex public key", async () => {
...
Deno.test("PrivateKeySigner - Error Handling", async (t) => {
  ...
  await t.step("should handle null/undefined in constructor", () => {
    assertThrows(() => new PrivateKeySigner(null as any), Error);

    assertThrows(() => new PrivateKeySigner(undefined as any), Error);
  });
