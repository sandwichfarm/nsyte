...
export async function createTarGzArchive(
  ...
  const compressedDataRaw = await Deno.readFile(`${tempTarPath}.gz`);
  const compressedData = compressedDataRaw.slice() as ByteArray;
...
export async function calculateSha256(data: Uint8Array): Promise<string> {
  // Ensure ArrayBuffer-backed view for stricter BufferSource typing
  const view = data.slice() as ByteArray;
