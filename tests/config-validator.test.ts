...
import { validateConfig, validateConfigWithFeedback, suggestConfigFixes } from "../src/lib/config-validator.ts";
...
Deno.test("Config Validator - With feedback and suggestions", () => {
  ...
  // Check suggestions
  assertEquals(feedback.suggestions.includes("Relay URLs must start with 'wss://' or 'ws://'"), true);
  assertEquals(feedback.suggestions.includes("Server URLs must start with 'https://' or 'http://'"), true);
  assertEquals(feedback.suggestions.includes("Bunker public key must be a 64-character hex string"), true);
});
