...
Deno.test("Init Command - Success Path", async (t) => {
  await t.step("should initialize project with private key", async () => {
    ...
    try {
      ...
      const hasSuccessMessage = logCalls.some(log => log.includes("Project initialized successfully"));
      ...
      const hasConfigPath = logCalls.some(log => log.includes("Configuration saved to .nsite/config.json"));
  ...
  await t.step("should initialize project with bunker connection", async () => {
    ...
    try {
      ...
      // Verify console output
      const logCalls = consoleLogSpy.calls.map(call => call.args.join(' '));
      const hasBunkerAuth = logCalls.some(log => log.includes("Authentication: bunker connection"));
